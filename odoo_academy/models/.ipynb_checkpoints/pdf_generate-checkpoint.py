...
class AccountMovePdf(models.Model):
    ...
    def pdf_generator(self):
        invoices = self.env['account.invoice'].search([('move_type', '=', 'out_invoice')])
        for invoice in invoices:
            self.pdf_invoice = self.env['account.invoice'].sudo().render_qweb_pdf([invoice.id], 'account.report_invoice_document')